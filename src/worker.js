...
async function process_create_idea_job( job ) {
    let card = {}
...
async function process_create_feature_job( job ) {
    let card = {}
    return card
}

async function process_update_job( job ) {
    let progress = 0;
...
function start() {
	...
	if (job.action == 'update') {
		...
		card = await process_create_idea_job( job )
	    } else if (job.aha_type == 'feature') {
		card = await process_create_feature_job( job )
	    }
	} else {
	    job.fail();
	}
